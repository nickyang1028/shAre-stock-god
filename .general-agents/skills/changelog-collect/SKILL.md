...
description: 收集 share-stock-god两个版本之间的 PR 信息并整理 changelog 草稿，更新到 CHANGELOG.md时使用。适用于收集 changelog、生成 changelog、更新 changelog、版本对比等场景。
---
...
帮助开发者收集 share-stock-god 两个版本之间的 Changelog，处理临时文件并更新到 CHANGELOG.md 文件中。
...
**提取后必须按照 changelog 规范进行校验，不合规则改写**（见下方"Changelog 合规校验"）。
...
| **新功能必须描述功能用途** | `🔥 Add BorderBeam component for animated border beam effect along container edges.` | `🔥 Add BorderBeam component.` |
...
- Commit: fix: Select height issue
- Category: Select
- Detail: 修复 Select value 为空时高度不正确的问题
```
...
- **同一功能有 2 条及以上改动时**，使用 `- 功能名` 作为分类标题，改动条目缩进列在其下
...
```
1. 选择目标分支（master/feature/自定义）
   ↓
2. **立即确定版本号**（minor/patch/自定义）
   ↓
3. 初始化 ~changelog.md 头部（写入版本信息）
   ↓
4. git log 获取两个版本间的 commits，提取所有 PR 编号
   ↓
5. **并行 dispatch subagents** 批量获取 PR 详情（每批最多 10 个）
   ↓
6. 汇总所有 PR 结果，逐条处理：
...
12. 预览确认（交互确认）
    ↓
13. 可选：更新 package.json 版本（npm version）
    ↓
14. 写入 CHANGELOG.md
    ↓
15. **运行 `npm run lint:changelog` 校验，有报错则修正并重新写入**
    ↓
16. 清理临时文件 ~changelog.md
```
...
- 写入前必须预览确认
- 保持中英文同步更新
- 描述以动作开头（如 `修复 Select ...`、`Fix Select ...`）
